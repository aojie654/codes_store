...
<body>
	...
	<form name=a action="<%=request.getContextPath()%>/servlet/Index?requestType=register"
		...
		<table width="650" align="center">
			...
			<tr>
				...
				<td width="45%">
					...
					<%if(request.getAttribute("alert")!=null){%><%=request.getAttribute("alert")%>
					<%} %>
				</td>
			...
			<tr>
				<td> 性 别:</td>
				<td><input type="radio" name="gender" value="1" checked="checked">男 <input
			...
			<tr>
				<td> 年 龄:</td>
				<td> <input type="text" name="age" size="20" onblur=isAge(this.value)></td>
				<td class="note">*请填写您的年龄</td>
			</tr>
			<tr>
				<td>User Email: </td>
				<td><input type="text" name="email" size="20" onblur=isEmail(this.value)></td>
			...
			<tr>
				<td>注册时间:</td>
				<td><%Calendar c = Calendar.getInstance();%><%=c.get(Calendar.YEAR)%>-<%=c.get(Calendar.MONTH)%>-<%=c.get(Calendar.DATE)%>
				</td>
			</tr>
			<tr>
