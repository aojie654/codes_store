...
<body>
	...
	<center>
		<table border="1" width=750>
			...
			<tr>
				<td rowspan="10" colspan="2"><img width="300"
			...
			<tr>
				<td colspan="2" class="bookInfo"><a href="#" class="common"
					onClick="window.close()"> 关闭窗口</a></td>

				<%
			User userCheck = (User) session.getAttribute("userInfo");
			if (userCheck!=null){ %>

				<td colspan="2" class="bookInfo"><a href="#"
					onClick="window.opener.location.href='<%=request.getContextPath() %>/servlet/Customer?requestType=addCart&bookId=<%=book.getId() %>';">加入购物车</a></td>

				<%}else{ %>
				<td colspan="2" class="bookInfo">购物请登录</td>
